<footer class="py-4 bg-light mt-auto border">
    <div class="container-fluid px-4">
        <div class="d-flex align-items-center justify-content-between small">
            <div class="text-muted">Copyright &copy; Qurrotul A'yun <?= date('Y') ?></div>
        </div>
    </div>
</footer>
<!-- jangan tutup div di sini jika sudah ditutup sebelumnya -->
...
window.addEventListener('DOMContentLoaded', event => {
    ...
    if (sidebarToggle) {
        sidebarToggle.addEventListener('click', event => {
            ...
            localStorage.setItem('sb|sidebarToggle', document.body.classList.contains(
                'sb-sidenav-toggled'));
        });
    }
});
